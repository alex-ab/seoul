...
private:
 ...
 public:
	...
	bool receive(MessageIrqNotify &msg)
	{
		...
		auto const irq = _cfgspace[15] & 0xff;
	...
	_vf(parent_bdf != 0), _map_mode(map_mode)
{
	...
	/* re-write IRQ we got potentially from the ACPI tables via get_gsi */
	if (!_msix_cap && !_msi_cap && _irq_count) {
		_cfgspace[15] &= 0xffffff00u;
		_cfgspace[15] |= _host_irqs[0];
	}
	...
	Logging::printf("%x:%x.%u direct PCI - %s\n",
	                (_guestbdf >> 8) & 0xff, (_guestbdf >> 3) & 0x1f, _guestbdf & 0x7,
	                _msix_cap ? "MSIX" : _msi_cap ? "MSI" : "GSI");
}
