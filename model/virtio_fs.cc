...
void Virtio_fs::notify (unsigned queue)
{
	...
	bool inject = used_queue.consume([&] (auto const &desc0, auto) {
		...
		case FUSE_READDIR: res = fuse_op_readdir(desc1, desc3, in.nodeid); break;
                                    ...
                                    uint64 const nodeid)
{
	...
	if (read_in.offset)
		Logging::printf("---------- partial read dir not supported\n");
...
//		if (_verbose)
		...
		/* FUSE_DIRENT_ALIGN in Linux */
		ret_size += msg.align(int32(sizeof(entry) + entry.namelen), 8);

		entry.off  = ret_size; /* next direntry for us to track ? */

		if (ret_size >= msg.buffer.offset)
