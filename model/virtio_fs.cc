...
void Virtio_fs::notify(unsigned queue, bool more)
{
	...
	bool inject = used_queue.consume([&] (auto const &desc0, auto) {
		...
		if (!response || response_size < sizeof(fuse_out_header)) {
			Logging::printf("virtio_fs, invalid response %p %u\n", response, response_size);
			return 0U;
		}
                                     ...
                                     int32          &err)
{
	...
	Logging::printf("%s: nodeid=%llx size=%llx name_size=%u name='%s'\n", __func__,
	                nodeid, attr_in.size, name_size, name_ptr);
                                  ...
                                  Desc         &out   , int32          &err)
{
	...
	if (_verbose)
		Logging::printf("%s: write in fh=%llx off=%llx size=%lx "
		                "write_flags=%lx lock_owner=%llx flags=%lx\n",
		                __func__,
		                write_in.fh,
		                write_in.offset,
		                write_in.size,
