...
namespace Seoul {
	...
	typedef	Genode::Constructible<Platform::Device::Irq> Host_irq;
	typedef	Genode::Constructible<Platform::Device>      Host_device;
	typedef Genode::Constructible<Platform::Connection>  Host_platform;
}
...
class Seoul::Pci : public StaticReceiver<Seoul::Pci>
{
	private:
		...
		Motherboard      &_mb;
		Genode::Heap     &_heap;
		Seoul::Vm_memory &_gmem;
		Genode::Mutex     _mutex { };

		Host_platform    &_platform;
		Host_device       _host_dev { };
		Host_irq          _host_irq { };

		Device            _device { _env.ep(), _mb.bus_hostirq, _mutex };

		uint64_t const _host_bdf_base;
		unsigned       _host_bdf { ~0u }; /* invalid by default */
	...
	public:
		...
		Pci(Genode::Env &env, Motherboard &mb, Genode::Heap &heap,
		    Seoul::Vm_memory &memory, Host_platform &platform,
		    uint64_t bdf_base)
		:
			_env(env), _mb(mb), _heap(heap), _gmem(memory),
			_platform(platform),
			_host_bdf_base(bdf_base)
		{
			...
			if (_platform.constructed()) {
				unsigned dev_cnt = 0;

				_platform->with_node([&](auto const &devices) {
					devices.with_optional_sub_node("device", [&](auto const &dev) {
						auto bdf_str = dev.attribute_value("name", String<8>());
						log(" PCI host device: ", bdf_str);
						dev_cnt ++;
						...
						auto span = Span(bdf_str.string(), bdf_str.length());
						span.cut(':', [&](auto const &bus_span, auto const &dev_span) {
							parse_unsigned<unsigned>(bus_span, bus, 16);
							parse_unsigned<unsigned>(dev_span, devi, 16);
							dev_span.cut('.', [&](auto const &, auto const &func_span) {
								parse_unsigned<unsigned>(func_span, func);
							});
						});

						_host_bdf = bus << 8 | devi << 3 | func;
					});
				});

				if (dev_cnt > 1)
					error("too many PCI host devices for passthrough");
				else if (dev_cnt == 1) {
					_host_dev.construct(*_platform);
				}

				if (!_host_dev.constructed()) {
					error("no passthrough PCI device available - halt");
					_platform->with_node([&](auto const &devices) {
						error(devices);
					});
					sleep_forever();
		...
		bool receive(MessageIrqNotify &msg)
		{
			...
			if (msg.mask & (1 << 5)) {
				if (_device.irq.constructed())
					_device.irq->ack_irq();
				error("ack before ", _device.irq.constructed(), " ", _host_irq.constructed());
				if (_host_irq.constructed())
					_host_irq->ack();
				error("ack done");
		...
		bool receive(MessageHwPciConfig &msg)
		{
			...
			auto const offset   = ((msg.bdf >> 8) & 0xff) * 256
			                    + ((msg.bdf >> 3) & 0x1f) * 8
			                    + ((msg.bdf >> 0) & 0x07);
			auto const cfg_addr = _host_bdf_base + 0x1000u * offset;
			...
			if ((_host_bdf & 0x7) && msg.bdf == (_host_bdf & ~7ul)) {
				log(" permit access to first function of device");
				_dump_msg(msg);
			} else if (msg.bdf != _host_bdf)
				return false;
...
//				if (msg.value != ~0u)
		...
		bool receive(MessageHostOp &msg)
		{
			...
			case MessageHostOp::OP_ASSIGN_PCI:
				...
				break;
			case MessageHostOp::OP_ATTACH_MSI:
				error("op assign msi XXX", msg.desc, " ", msg.is_hpet);
				// -> return values in msg.msi_address, msg.msi_value
				// XXX msg.valu -> _device.irq_nr XXX
				msg.value = 0x7a; /* XXX */
				[[fallthrough]];
			case MessageHostOp::OP_ATTACH_IRQ: {
				...
				if (_platform.constructed()) {
					_host_irq.construct(*_host_dev);
					_host_irq->sigh_omit_initial_signal(_device.handler);
				} else {
		...
		bool receive(MessageAcpi &msg)
		{
			...
			/* have to be same as specified in dsdt.asl ! */
			switch(msg.pin + 1) {
			case 1: msg.gsi =  5; return true;
			case 2: msg.gsi = 10; return true;
			case 3: msg.gsi = 11; return true;
			case 4: msg.gsi = 13; return true;
			default:
				Genode::warning("unknown PIN ", msg.pin);
			}
